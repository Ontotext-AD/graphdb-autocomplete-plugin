...
class EntitiesIterator implements InputIterator {
    ...
    public BytesRef next() throws IOException {
        while (++currentIteratorIndex <= entities.size() && !autocompleteIndex.isShouldInterrupt()) {
            ...
            if (v instanceof IRI && !AutocompleteIndex.SPECIAL_ENTITIES.contains(v.stringValue())) {
                if (autocompleteIndex.isRepositoryCleared()) {
                    // If repository is cleared only entities for which there is a statement
                    // should go into index. See https://ontotext.atlassian.net/browse/GDB-4067
    ...
    private BytesRef getBytesRefFromValue(Value v) {
        currentURI = (IRI) v;
